...
from models import get_player, get_state, get_autonomy, get_region
...
def get_region_info(user, id):
    try:
        ...
        upper = user.driver.find_element(By.CSS_SELECTOR, "div.margin > h1 > span").get_attribute('action').split('/')
        ...
        for div in data:
            if "Governor:" in div.find_element(By.CSS_SELECTOR, "h2").text:
                ...
                autonomy.set_budget('money', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/1"]').text.split(' ')[0]))
                autonomy.set_budget('gold', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/2"]').text.split(' ')[0]))
                autonomy.set_budget('oil', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/3"]').text.split(' ')[0]))
                autonomy.set_budget('ore', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/4"]').text.split(' ')[0]))
                autonomy.set_budget('uranium', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/11"]').text.split(' ')[0]))
                autonomy.set_budget('diamonds', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/15"]').text.split(' ')[0]))
...
def get_state_info(user, id):
    try:
        ...
        state = get_state(id)
        state.set_budget('money', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/1/state"]').text.split(' ')[0]))
        state.set_budget('gold', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/2/state"]').text.split(' ')[0]))
        state.set_budget('oil', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/3/state"]').text.split(' ')[0]))
        state.set_budget('ore', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/4/state"]').text.split(' ')[0]))
        state.set_budget('uranium', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/11/state"]').text.split(' ')[0]))
        state.set_budget('diamonds', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{id}/15/state"]').text.split(' ')[0]))
...
def get_autonomy_info(user, id):
    try:
        ...
        autonomy = get_autonomy(id)
        autonomy.set_state(get_state(user.driver.find_element(By.CSS_SELECTOR, "div.margin > h1 > span").get_attribute('action').split('/')[-1]))
        ...
        for div in data:
            if "Governor:" in div.find_element(By.CSS_SELECTOR, "h2").text:
                ...
                for region in div.find_elements(By.CSS_SELECTOR, "slide_profile_data"):
                    print(region)
                    regions.append(get_region(dotless(region.get_attribute('action').split('/')[-1])))
                autonomy.set_regions(regions)
        if len(autonomy.regions):
            regionid = autonomy.regions[0].id
            autonomy.set_budget('money', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{regionid}/1"]').text.split(' ')[0]))
            autonomy.set_budget('gold', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{regionid}/2"]').text.split(' ')[0]))
            autonomy.set_budget('oil', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{regionid}/3"]').text.split(' ')[0]))
            autonomy.set_budget('ore', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{regionid}/4"]').text.split(' ')[0]))
            autonomy.set_budget('uranium', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{regionid}/11"]').text.split(' ')[0]))
            autonomy.set_budget('diamonds', dotless(user.driver.find_element(By.CSS_SELECTOR, f'span[action="graph/balance/{regionid}/15"]').text.split(' ')[0]))
        return_to_mainpage(user)
        return True
    except Exception as e:
